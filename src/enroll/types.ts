...
// pinta-manager's `sidecar/src/enroll/enroll-source.ts` (troy §4.2 applied to
// enrollment: each wrapper owns "what lives where" for ITS host tool; the
// manager sidecar owns only generic engines). The sidecar dynamically
// `import()`s this adaptor's built `dist/index.mjs` and drives
// `export const enroll: EnrollSource`. Do NOT diverge from the sidecar copy —
// when @pinta-ai/core ships the contract, this file is deleted and imports
// are repointed. Only `node:*` primitives in the type surface (Node 20 + Bun).
...
/**
 * Resolve a `Record<key, TokenSource>` catalog mapping to actual values.
 * e.g. { OTEL_EXPORTER_OTLP_TRACES_ENDPOINT: 'relay-endpoint' }
