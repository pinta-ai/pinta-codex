...
class RetryQueue {
    ...
    enqueue(payload) {
        try {
            ...
            process.stderr.write(`[pinta-codex] retry-queue enqueue failed: ${err}\n`);
        }
    }
    /**
    ...
    rewrite(entries) {
        try {
            ...
            process.stderr.write(`[pinta-codex] retry-queue rewrite failed: ${err}\n`);
        }
    }
    /**
    ...
    tryAcquireLock() {
        ...
        while (Date.now() - start < LOCK_TIMEOUT_MS) {
            try {
                ...
                if (err?.code !== "EEXIST") {
                    process.stderr.write(`[pinta-codex] retry-queue lock open failed: ${err}\n`);
                    return false;
                }
    ...
    trim() {
        ...
        process.stderr.write(`[pinta-codex] retry-queue full, dropping ${drop} oldest entries\n`);
