#!/usr/bin/env node
// M5a — Node 20 smoke test for the ESM dual-entry (dist/index.mjs), mirrors
// pinta-cc's A3 smoke:import script.
//
// Two things must hold:
//   1. `lifecycle` is exported and `lifecycle.id === 'pinta-codex'`.
...
if (mod.lifecycle.id !== "pinta-codex") {
  console.error(
    `[smoke:import] FAIL: lifecycle.id = ${JSON.stringify(mod.lifecycle.id)}, expected "pinta-codex"`,
  );
  process.exit(1);
}
...
console.log(
  "[smoke:import] OK: import()-ing dist/index.mjs did not run the hook, and lifecycle.id === 'pinta-codex'",
);
process.exit(0);
